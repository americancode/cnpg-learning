...
kubectl -n migration-db wait --for=condition=Ready certificate/minio-tls --timeout=5m
  ...
  containers:
    - name: minio-mc
      ...
      command:
        - sh
        - -ec
        - mc --insecure mb --ignore-existing local/crunchy-cnpg-migration
      env:
        - name: MC_HOST_local
          value: https://minioadmin:minioadmin123@minio.migration-db.svc.cluster.local:9000
        - name: HOME
          value: /tmp
      volumeMounts:
        - name: tmp
